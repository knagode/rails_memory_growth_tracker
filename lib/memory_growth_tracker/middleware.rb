...
module MemoryGrowthTracker
  class Middleware
    ...
    def self.redis_tracked_requests_key
      "#{MemoryGrowthTracker.configuration.redis_key_prefix}:tracked_requests"
    end
    ...
    def self.reset_data
      @@request_count = 0
      @@skipped_requests_count = 0
      MemoryGrowthTracker.configuration.redis.del(redis_tracked_requests_key)
    end

    def self.tracked_requests_count
      MemoryGrowthTracker.configuration.redis.get(redis_tracked_requests_key)&.to_i || 0
    end
    ...
    def config
      MemoryGrowthTracker.configuration
    end
